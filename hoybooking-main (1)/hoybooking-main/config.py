...
class Config:
    ...
    # Local default is SQLite for zero-config startup.
    # Set HOYBOOKING_DB_BACKEND=mysql (or HOYBOOKING_USE_MYSQL=true) to force MySQL.
    _db_backend = (os.environ.get("HOYBOOKING_DB_BACKEND") or "").strip().lower()
    _use_mysql = _db_backend == "mysql" or _is_truthy(os.environ.get("HOYBOOKING_USE_MYSQL", ""))
    ...
    if _raw_url:
        ...
        SQLITE_PATH = os.path.join(os.path.dirname(__file__), "instance", "hoybooking.db")
